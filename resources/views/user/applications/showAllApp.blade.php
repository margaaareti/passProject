                        ...
                        <div class="card__body card-body">
                            ...
                            <div class="approved-info">
                                @if($application->is_approved === true)
                                    <p class="approved-info__text">Одобрено <img class="approved-info__image" src={{asset('img/approvedAppIcon.png')}} alt="Картинка"></p>
                                @else
                                    <p class="approve-info__text">
                                        На рассмотрении <img class="approved-info__image" src={{asset('img/wwww.svg')}} alt="Картинка"></p>
                                @endif
                            </div>
                            ...
                            @if ($application->guests)
                                ...
                                <ul class="card-body__list guest-list">
                                    ...
                                    @foreach ($application->guests as $index => $guest)
                                        <li class="card-body__text @if($index >= 2) hidden @endif">{{ $guest->name }}</li> <!-- Показываем всех гостей -->
                                    @endforeach
                                </ul>

                                @if($guestCount > 2) <!-- Если гостей больше 3, показываем кнопку "Скрыть всех" -->
                                <button class="card-body_button" onclick="showAllGuests(this)">Показать всех</button>
                                @endif
                            ...
                            @elseif ($application->cars)
                                ...
                                <ul class="card-body__list car-list">
                                    ...
                                    @endforeach
                                    @if($carsCount > 2) <!-- Если гостей больше 3, показываем кнопку "Скрыть всех" -->
                                    <button class="card-body_button" onclick="showAllGuests(this)">Показать всех</button>
                                    @endif
                                </ul>
                            @endif
